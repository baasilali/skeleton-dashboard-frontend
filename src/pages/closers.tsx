...
function ClosersBody() {
  const { filters } = useDashboardFilters();
  const kpis = React.useMemo(() => calculateKPIs(filters), [filters]);

  const closeRateTrendData = React.useMemo(
    () => getCloseRateTrendChartData(filters),
    [filters]
  );
  ...
  const closerCommission = React.useMemo(
    () => getCloserCommission(filters),
    [filters]
  );

  const closerTableData = React.useMemo(
    () => getCloserTableData(filters),
    [filters]
  );
        ...
        <SidebarInset>
          ...
          <div className="flex flex-1 flex-col gap-4 p-4 mt-4">
            <div className="flex flex-1 flex-col gap-4">
              ...
              <MetricCard
                  description="Closer Commission"
                  value={currency.format(closerCommission)}
                  delta={12}
                />
                <MetricCard
            ...
            <div className="flex flex-1 flex-col gap-4 mt-4">
              ...
              <div className="grid auto-rows-min gap-4 md:grid-cols-1">
                <InteractiveLineChart title="Close Rate Trend" data={closeRateTrendData} />
              </div>
              <div className="grid auto-rows-min gap-4 md:grid-cols-2">
                <SetterCloserTable data={closerTableData} title="Closer Performance" />
